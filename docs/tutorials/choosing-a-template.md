...
## Multi-agent evaluation (Layer C)

Use `multi-ai-cli` when you want **several agent CLIs in one workspace** to compare providers.

- Installs Claude Code, Grok Build, Pi, Hermes, Codex, Gemini CLI, and OpenCode
- Agent-agnostic security floor: `non-root-enforcer`, `ai-agent-sandbox` (moderate), `container-firewall` (`services: multi-ai`)
- Shared MCP via `mcp-server-manager` (not Claude-only MCP features)
- **Auth is multi-key**: each vendor needs its own API key or login
- Prefer dedicated Layer A (Claude) or Layer B (single-agent) templates for daily work
